...
int uthread_spawn(thread_entry_point entry_point)
{
  ...
  int tid= orchestrator->spawn(entry_point);
  if (tid == -1)
  {
...
{
  int ret= orchestrator->terminate(tid);
  if (ret == -1)
  {
...
{
  int res= orchestrator->block(tid);
  if (res == -1)
  {
    std::cerr
        << "thread library error: thread does not exist or cannot be blocked"
        << std::endl;
  }
  return res;
}
 ...
 * error. When a thread transition to the READY state it is placed at the end of
 * the READY queue.
 *
 * @return On success, return 0. On failure, return -1.
 */
...
{
  int res= orchestrator->resume(tid);
  if (res == -1)
  {
    std::cerr
        << "thread library error: thread does not exist or cannot be resumed"
        << std::endl;
  }
  return res;
}
 ...
 */
int uthread_get_tid() { return orchestrator->get_current_thread(); }
 ...
 */
int uthread_get_total_quantums() { return orchestrator->get_total_quantums(); }
...
{
  int num_quantums= orchestrator->get_quantums(tid);
  if (num_quantums == -1)
  {
    std::cerr << "thread library error: thread does not exist" << std::endl;
  }
